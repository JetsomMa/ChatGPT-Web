...
export class MidjourneyMessage {
  private magApiQueue = CreateQueue(3)
  ...
  async RetrieveMessages(limit = this.config.Limit) {
    ...
    const response = await fetch(`https://discord.com/api/v10/channels/${this.config.ChannelId}/messages?limit=${limit}`, fetchOptions)
    if (!response.ok) {
